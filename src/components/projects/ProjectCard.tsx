...
export function ProjectCard({ project, index }: ProjectCardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group/card flex flex-col rounded-xl border border-border/60 bg-card/80",
    ...
    >
      ...
      <Link
        ...
        className="relative block h-44 overflow-hidden rounded-t-xl bg-gradient-to-br from-blue-950/60 via-indigo-950/40 to-background sm:h-48"
      >
      ...
      <div className="flex flex-1 flex-col gap-3 p-5 sm:p-6">
        <Link href={`/projects/${project.id}`} className="group/title">
          <h3 className="text-lg font-semibold text-foreground transition-colors group-hover/title:text-blue-200">
            {project.title}
          </h3>
        </Link>
