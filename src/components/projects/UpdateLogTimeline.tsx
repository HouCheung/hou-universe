    ...
    <div className="relative">
      ...
      <div className="flex flex-col gap-8">
        {entries.map((entry, index) => (
          ...
          >
            {/* Node dot */}
            <div className="absolute left-[7px] top-1.5 h-2.5 w-2.5 rounded-full border-2 border-slate-400 bg-background ring-4 ring-background sm:left-[11px]" />

            {/* Content */}
            <div className="flex-1 rounded-lg border border-border/50 bg-card/60 p-4 backdrop-blur-sm transition-all duration-300 hover:border-slate-400/25 hover:bg-card sm:p-5">
              ...
              <ul className="mt-2 space-y-1.5">
                {entry.changes.map((change, ci) => (
                  ...
                  >
                    <span className="mt-[0.35em] block h-1.5 w-1.5 shrink-0 rounded-full bg-slate-400/40" />
                    {change}
                  </li>
                ))}
              </ul>
